...
public class Interface {
	...
	public static void main(String[] args) {
		...
		float[][][] deblurred = Algorithms.deblurBasic2Switch(originalBlur, 1f, 64f, 1, true);
		//float[][][] deblurred = Algorithms.richardsonLucySwitch(originalBlur, 6f, 15, true);
		...
		image1 = Algorithms.richardsonLucySwitch(image1, 91.25f, 1, true);
		...
		/* Optimization of a function by NRMSE
		final BufferedImage rawImageBlur = loadImageFromFile(new File("src/deconvolution/Image Radius 12.png"));
		final BufferedImage rawImageOrig = loadImageFromFile(new File("src/deconvolution/Image Original.jpg"));
		float[][][] originalBlur = Algorithms.imageToArray(rawImageBlur);
		float[][][] originalOrig = Algorithms.imageToArray(rawImageOrig);
		double bestNRMSE = Double.MAX_VALUE;
		for (int i = 0; i < 5000; i++) {
			float weight = (float)((Math.random() - 0.5) * 0.5 + 3.84);
			float ratio1 = (float)((Math.random() - 0.5) * 0.4 + 1.52);
			float ratio2 = (float)((Math.random() - 0.5) * 0.4 + 0.52);
			float amountOffset = (float)((Math.random() - 0.5) * 0.5 + 2.59);
			float[][][] deblurred = Algorithms.deblurBasic2TEST(originalBlur, originalBlur, amountOffset, 12, ratio1, ratio2, weight);
			double nrmse = Algorithms.normalizedRootMeanSquareError(originalOrig, deblurred);
			
			if (nrmse < bestNRMSE) {
				bestNRMSE = nrmse;
				print((float)nrmse + " " + weight + " " + ratio1 + " " + ratio2 + " " + amountOffset);
				// For radius  4: 20.470316 1.6496575 1.7802963 0.37898126 1.355493
				// For radius 12: 25.16101 3.9350662 1.5190495 0.50445396 2.640755
			}
		}
		//*/
	...
	static void redrawPreviewImage() {
		...
		if (!Algorithms.useOpenGL) {
			...
			Interface.setProcessName("OpenGL Deblurring (" + ((endTime - startTime) / 1000000) + "ms)");
			Interface.updateProgress(1.0);
		}
		...
		if (previewImage != null) {
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					...
					DeblurOpenGL.setImageParameters(previewImage.getWidth(), previewImage.getHeight(), 12); // Deblur radius here is ignored
	...
	private static void setupFrame() {
		...
		frame = new JFrame("Fast Deblur Algorithms - Daniel Williams 2023");
		...
		sharpenButton.setToolTipText("Laplacian filter");
		...
		deblurButton3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ImageEffects.showEffectDialogue("Fast Deblur", ImageEffects.FAST_METHOD,
						...
						"Radius", 1, Algorithms.useOpenGL ? 60 : 200, 5, 4,	  // OpenGL implementation can only go up to a radius of 64 pixels.
